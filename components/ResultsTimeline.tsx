...
export const ResultsTimeline: React.FC<ResultsTimelineProps> = ({ actions, annotations = [], narrativeSteps, currentTime = 0, onSeek, learnedContext, videoUrl, duration, cleanedOutput }) => {
  ...
  const downloadPlaywright = () => {
    ...
    script += `  // Semantic locators do not depend on viewport size.\n`;
    script += `  await page.setViewportSize({ width: 1280, height: 720 });\n\n`;
    ...
    narrativeSteps.forEach((step, idx) => {
      ...
      linkedActions.forEach(action => {
          ...
          if ((action.action_type === 'click' || action.action_type === 'double_click' || action.action_type === 'right_click') && action.target) {
              const clickMethod = action.action_type === 'double_click' ? 'dblclick()' : action.action_type === 'right_click' ? 'click({ button: "right" })' : 'click()';
              script += `    // Semantic Locator: ${action.target.container} -> ${action.target.relation_to_landmark} '${action.target.nearest_landmark}'\n`;
              let role = action.target.element_type;
              if (role === 'other' || role === 'icon' || role === 'video' || role === 'canvas') role = 'button';
              else if (role === 'input') role = 'textbox';
              else if (role === 'dropdown') role = 'combobox';
              else if (role === 'menu_item') role = 'menuitem';
              else if (role === 'image') role = 'img';
              else if (role === 'list_item') role = 'listitem';
              else if (role === 'modal') role = 'dialog';
              else if (role === 'panel') role = 'region';
              else if (role === 'toggle') role = 'switch';
              
              if (role === 'text') {
                script += `    await page.getByText('${action.target.element}').${clickMethod};\n`;
              } else {
                script += `    await page.getByRole('${role}', { name: '${action.target.element}' }).${clickMethod};\n`;
              }
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            ...
            {/* Semantic Locator Badge */}
            {action.target?.container && !isError && (
              <span className="flex items-center gap-1 text-[10px] font-mono text-blue-600 dark:text-blue-400/70 bg-blue-50 dark:bg-blue-900/20 px-1.5 py-0.5 rounded-md border border-blue-200 dark:border-blue-900/50 cursor-help" title={`Container: ${action.target.container}\nRelation: ${action.target.relation_to_landmark} '${action.target.nearest_landmark}'`}>
                <Target size={12} />
                {action.target.element_type} in {action.target.container}
              </span>
            )}
           ...
           >
             ...
             {isExportDropdownOpen && (
               <div className="absolute right-0 mt-1 w-36 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg transition-all z-50 overflow-hidden">
                 <button onClick={() => { downloadJSON(); setIsExportDropdownOpen(false); }} className="w-full text-left px-3 py-2 text-[11px] text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700">Export Raw ({actions.length})</button>
