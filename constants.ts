...
7. **MONOTONIC TIMING:** For steps with linked actions, use the timestamp of the FIRST (earliest) linked action. For standalone conceptual steps, use the audio timestamp BUT it MUST be equal to or later than the previous step's timestamp — never go backwards. If the audio timestamp would regress, use the previous step's timestamp instead.
