# Pass 2: Post-Loop Cleanup: Sort, Orphan Merge, Schema Normalization, Annotation Filter
...
The exclusion list covers all non-procedural `InsightType` values (see `types.ts` line 50: `'explanation' | 'rationale' | 'tip' | 'warning' | 'workflow_framing' | 'comparison'`). These are standalone by design — they provide context, tips, or framing rather than describing user actions. Only `'explanation'` steps (the default procedural type) are merge candidates. This extends the existing coverage validation exemption (which already skips `'rationale'` steps when counting unlinked steps) to all non-procedural types.
...
Reassign `state.narrativeSteps = cumulativeNarrative` after the filter (the `.filter()` produces a new array, so the old reference on `state.narrativeSteps` is stale and must be updated).
...
|-------|----------------------|---------|-----------|
| `is_error_recovery` | `boolean \| undefined` (optional, line 88) | `false` | Most actions are not error recovery |
| `interacted_components` | `UIComponent[] \| undefined` (optional, line 84) | `[]` | Empty array, not null — type is array-or-undefined |
| `context_note` | `string \| null` (required, nullable, line 81) | `null` | The TypeScript type is `string \| null` (not `string \| undefined`), making `null` the canonical absent value. Note: both `null` and `""` are stripped by `stripEmptyAndDefaults`, so this only affects the raw export shape. |
| `confidence` | `ActionConfidence` (required, line 82) | `'high'` | Matches the existing `GLOBAL_DEDUPLICATION_PROMPT` Rule 8 which states `"confidence": "high" (default if not set)` |

**Do NOT set** `input_data` — the type is `InputData | undefined` (optional, not nullable, line 86). Setting it to `null` would violate the TypeScript type. Leaving it `undefined` (absent) is correct and `stripEmptyAndDefaults` handles it.

### Relationship to Phase D Rule 8

Phase D's `GLOBAL_DEDUPLICATION_PROMPT` Rule 8 instructs the LLM to normalize schema during the dedup pass (e.g., `"context_note": ""`). This deterministic normalization serves as a **fallback for the Phase D failure path** (catch block) where the LLM never ran, and as a **consistency layer on the success path** to fill any fields the LLM missed. On the success path, Phase D Rule 8 uses `""` for context_note while this normalization uses `null` — since `cleanFinalOutput` strips both via `stripEmptyAndDefaults`, the cleaned export is identical. For the raw export, `null` is type-correct.
...
**Insert location:** `server/jobManager.ts`, in the per-chunk annotation processing section. The filter must run **before** the `.map()` that assigns new IDs to `validated_segment_annotations`.
...
The implementation must support logging the filter count, so **do not chain the filter directly into the `.map()` call**. Instead, break it into separate statements:

```typescript
// Example structure (adapt to actual variable names):
const rawAnnotations = phaseBResult.result.validated_segment_annotations || [];

// Remove empty/whitespace-only and placeholder annotations
const filteredAnnotations = rawAnnotations.filter(ann => {
  if (!ann.content?.trim()) return false;          // empty or whitespace-only
  if (PLACEHOLDER_RE.test(ann.content.trim())) return false;  // placeholder stub
  return true;
});

const placeholderCount = rawAnnotations.length - filteredAnnotations.length;
if (placeholderCount > 0) {
  addLog('info', `Filtered ${placeholderCount} placeholder annotations from chunk.`);
}

const newValidatedAnnotations = filteredAnnotations.map(ann => { /* existing ID assignment */ });
```

> **Note:** Neither the empty-content check nor the placeholder check exist in the current code — both are new additions.
...
- Do **NOT** filter by `annotation_type === 'info'` — the `AnnotationType` union in `types.ts` (lines 3-12) does not include `'info'` as a named variant, but the `| string` catch-all allows it. Filtering by type alone would remove valid annotations
...
| File | Changes |
|------|---------|
| `server/jobManager.ts` | All three changes. Add `normalizeActionDefaults` helper. Add placeholder regex constant. Add annotation filter before ID assignment in chunk loop. Add post-loop sort + orphan merge block. |
